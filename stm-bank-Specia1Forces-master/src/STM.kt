...
class TxVar<T>(initial: T) {
     ...
     */
    /*
    Открывает эту транзакционную переменную в указанной транзакции [tx] и применяет к ней функцию обновления [update].
    ...
    fun openIn(tx: Transaction, update: (T) -> T): T {
        ...
        while (true) {
            ...
            var curValue: T
            if (curLoc.owner === tx) {
                curValue = curLoc.newValue as T
            } else {
                curValue = when (curLoc.owner.status) {
            ...
            val updValue = update(curValue)//curLoc.oldValue
